...
class absenteeism_model():
    ...
    def __init__(self, model_file, scaler_file):
        # read the 'model' and 'scalar' files which were saved
        with open('model','rb') as model_file, open('scaler','rb') as scaler_file:
            self.reg = pickle.load(model_file)
            self.scaler = pickle.load(scaler_file)
            self.data = None

    # take a data file (*.csv) and preprocess it in the same way as in lectures
    def load_and_clean_data(self,data_file):

        # import the data
        df = pd.read_csv(data_file,delimiter=',')
        # store the data in a new variable for later use
        self.df_with_preedictions = df.copy()
        # drop the 'ID' column
        ...
        # to avoid multicollinearity, drop the 'Reason for Absence' column
        ...
        # note: there is a more universal version of this code, however the following will best suit our purposes
        ...
        column_names_upd = ['Reason 1', 'Reason 2', 'Reason 3', 'Reason 4', 'Month Value', 'Day of the Week', 'Transportation Expense', 'Distance to Work', 'Age',
       'Daily Work Load Average', 'Body Mass Index', 'Education',
       'Children', 'Pets', 'Absenteeism Time in Hours']
        ...
        # we need this line so we can use it in the next function
        self.data = self.scaler.transform(df)

    # a function which outputs the probability of a datapoint to be 1
    def predicted_probability(self):
        if (self.data is not None):
            pred = self.reg.predict_proba(self.data)[:,1]
            return pred

    # a function which outpits 0 or 1 based on our model
